...
def ban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    try:
        ...
        if excp.message == "Reply message not found":
            # Do not reply
            reply = "Nub {} Snapped by Pro {} in {}!".format(mention_html(member.user.id, member.user.first_name), mention_html(user.id, user.first_name), html.escape(chat.title)), quote=False)
            message.reply_text(reply, reply_markup=keyboard, parse_mode=ParseMode.HTML)
            return log
        else:
